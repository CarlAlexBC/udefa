...
import { AlertCircle, Check, Loader2, Pencil, Search, X } from 'lucide-react'
...
export default function ReactivosAdminPage() {
  ...
  // Reactivo abierto en el modal de edición (null = modal cerrado).
  const [editando, setEditando] = useState<Reactivo | null>(null)

  // Reemplaza un reactivo en la lista tras guardarlo, sin recargar todo.
  function reemplazarEnLista(actualizado: Reactivo) {
    setReactivos((prev) =>
      prev.map((r) => (r.id === actualizado.id ? { ...r, ...actualizado } : r)),
    )
  }
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Explora el banco por fase, tema y polaridad, y edita cualquier
          reactivo con el botón de la derecha.
        </p>
      </div>
...
/**
 * Modal de edición de un reactivo.
 *
...
function ModalEditarReactivo({
  ...
  // Los exámenes calificables exigen una opción marcada como correcta.
  const exigeCorrecta = ['psicometrico', 'cultural'].includes(
    reactivo.bloque.examen.tipo,
  )
  ...
  async function guardar() {
    ...
    setGuardando(true)
    try {
      const actualizado = await apiFetch<Reactivo>(
